...
#' and includes a model of model predictions across a key continuous variable (`plot_across`, defaults to "doy")
...
#' @param plot_across A single string specifying the variable to plot across. Defaults to "doy".
...
plot_seasonal_gam_panels = function(model, ## fitted gam model
  ...
  dat_pred = predict_gam(model, plot_across = plot_across,
  ...
  for(i in 1:nrow(panel_combinations)){
    ...
    if(plot_coverage){
      dat_rugs = dplyr::left_join(panel_combinations[i, ],
                           dat_model,
                           by = predictors_panel) |>
        dplyr::group_by(dplyr::across(dplyr::all_of(c(predictors_panel, plot_across)))) |>
        dplyr::summarize(n = dplyr::n(), .groups = "drop") |>
        dplyr::ungroup()

      hist_gp = dat_rugs |>
        # filter(_data[[facet_by]] == cur_panel$data[[facet_by]][1]) |>
        ggplot2::ggplot(ggplot2::aes(x = .data[[plot_across]], y = .data$n))+
